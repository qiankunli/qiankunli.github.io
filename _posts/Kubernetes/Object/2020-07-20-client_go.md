...
接口的http 形态：`http://apiserver/api/v1/namespaces/default/pods/$podName` ，可以 `kubectl get --raw /api/v1/namespaces/default/pods/$podName` 访问http 接口拿到pod 数据，为了方便 数据的访问， client-go 提供了RESTClient 来进行初步的封装。类似于 java 的HttpUtils 类。
