...
Infrastructure-as-Code，IaC 工具的一种。
...
oam 单从格式看，**划分为components/traits/policies/workflow几个大类，每个类别包含 name/type/properties 字段**。kebevela 作为中间层，将这些 运维参数 进行组装渲染，变成k8s的实际资源。
...
2. 同时用户如果想做统一的能力，如要把 Rollout 抽出来的概念变成公共的功能给 WebService 与 Knative Revision 使用，这种情况在 Helm 中无法实现，包括后期做统一的监控、统一的发布管理、统一的日志管理、统一的扩缩容等，Helm 均无法实现。但是在 KubeVela 中，基于 OAM 模型提供的公共标准，就可以实现一些公共的能力。
...
|翻译成k8s yaml|go template|cue|
|与k8s对象|一对一|oam 抽象转换|
|能力复用|chart 应用商店|xxDefinition|
