...
2023.1.10 感受：云原生有两个方面：平台侧如何改造，来提高硬件效率，提供能力？单纯k8s做的工作远远不够。应用侧如何改造，无状态服务可能好一些，有状态服务如何更saas化，比如db 如果不做存算分离，就很难称得上云原生的。
...
[腾讯百万级别容器云平台实践揭秘](https://mp.weixin.qq.com/s/DUTuauVUNuPuhMwZc_tqYQ) 
1. 在线业务资源容器化部署的问题与优化方案
  1. 资源利用率提升——动态压缩和超卖
  2. 节点负载均衡——动态调度和重调度。Pod-Problem-Detecor、NodeProblem-Detecor，检测出哪个 Pod 会导致节点高负载，哪些 Pod 属于敏感 Pod，通过事件上报告诉API Server，让调度器将异常 Pod、敏感 Pod 重新调度到空闲节点。
  3. K8s 业务弹性伸缩——协同弹性，HPAPlus-Controller，VPAPlus-Controller，
  4. 集群资源管理——动态配额和资源腾挪
2. 容器化对动态路由同步的挑战与解决方案。
    1. 难点：容器化之后容器的变更更加频繁，有的业务规模非常庞大，单个负载的 Pod 可能成千上万，还允许云上云下混合部署，跨集群多路由服务共享路由。对于运行阶段，例如容器异常自动重启，或者 Pod 其中一个容器通过原地生成的方式启动，这些场景就会绕过前面提到的滚动更新和删除保护。
    2. 当业务在销毁之前先剔除路由，业务在滚动更新的时候先保证上一批添加。将路由融入到 Pod 生命周期里
3. 一种全新的容器销毁失败自愈机制探索
  1. 容器销毁重建时老的容器要销毁，新的容器能创建出来。但实际上在生产环境中这并不是 100% 能保证的。当容器发生销毁失败时，能够定位问题（node日志、kubelet日志、pod status等），已经有解决方案的，能够触发自愈路径。同时，对于新增问题，我们可以缩短问题范围（确定不是已知问题），提高定位效率。PS：提到了决策树模型

  [云原生成熟度模型是什么（我为什么要关心）？](https://mp.weixin.qq.com/s/_jTKuJXXL0zH7Arh9GCW5A)