...
一个paas 包括以下几个部分
1. 工作流。工作流是一个偏上层的概念，先做镜像，先发哪再发哪，几个实例，再发几个实例，流量先切多少，再切多少。下一层负责当前阶段意图的具体实现，比如实例配置、发到哪个集群。workflow vs 手工管理
	1. 对于paas，就是镜像制作与分发、开发/测试环境部署、生产部署（多集群）
	2. 对于机器学习平台，就是样本生成、训练、评估、推理。 
2. 基础设施：多集群。
3. 应用管理
	1. 应用配置。包括label、运行时，所需资源等。
	2. 应用的载体，描述应用本身，deployment 不够，openkruise 的cloneset。
   	3. 应用的运维，比如hpa 等组件。一个应用会对应一个workload 集合。
