...
示例demo yaml 中的 read-app/apply-app/suspend 都是一个task type，有些是builtin的，有些是自定义扩展实现的。buildin的比较简单，初始化taskDiscover时直接写入，custom 则由 TaskLoader 负责管理
...
```
对于自定义 task type，从xxDefinition 中拿到cue 模版文件
```
TaskLoader.GetTaskGenerator
