...
集群资源充足，最小调度延迟；集群资源不足：
1. 管理员视角：优先吞吐量；优先级；最大资源利用率
2. 用户视角：最大调度延迟；
3. 机器视角：最大资源利用率
...
3. 比如job一共10个 minMember=5，如果已经伪分配了4个，第5找不到合适的节点。gang plugin 向Session 注册了 ReclaimableFn/PreemptableFn（计算牺牲作业）。对于 这种状态的 job，这4个task 可以被抢占后回收。
