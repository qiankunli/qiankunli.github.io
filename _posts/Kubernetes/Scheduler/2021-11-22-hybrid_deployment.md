...
2. 节点超卖，然后又有一系列辅助动作保证Pod QoS
  1. 节点不要成为热点（节点压力过大本身就会影响业务）
  2. 在线任务调度延迟不受大的影响：不受离线影响（优化os 抢占策略），不要被其它在线影响。 如何识别干扰源，如何压制干扰源？
  3. 也别让离线任务过的太惨（离线任务一直没资源跑就调离这个node）。
3. 资源池 ==> 本质是对节点上不同pod的分类管理，干脆提出一个资源池的概念。节点划分资源池，高优池支持绑核、cpu负载控制在50%以下，池间强隔离，不同池运行不同类型的任务，池资源大小随负载动态调整。
