...
在allocate.go 中，针对一个task 的调度，会先找到task 所属queue 富余的资源（包括cpu/memory/自定义），如果富余资源够 task 使用，则配额审核成功，开始进入预选优选环节。 
...
富余资源的计算来自ssn.UnderusedResources ，只有proportion 一个plugin 注册了 underUsedFns，所以主要工作 在 proportion plugin 里。
...
proportionPlugin.OnSessionOpen 主要工作就是 为deserved/allocated/request/capability 赋值
1. 其中capability 从配置获取
2. allocated/request 从queue 所属的 pod 获取
...
一次循环给 queue 属于它weight 配额的资源，够了就算了，不够等别人的配额分完了再来一轮。哪怕资源足够，proportion 也只是给queue 分配刚好够 request的资源。假设资源总量是100，total weight=10，那么每个queue 获取的资源则以10为unit，会存在一种可能：为queue1 分配了30 resource，但是queue1 实际用不到（碎片），为queue2 分配了10 resource但不够用。对于这个情况，allocate action+proportion plugin 无法为queue2 分配job 了， 轮到enqueue action 来检测集群资源是否 还有富余，进而给queue2 的未调度的job 一次机会。 

这其中定义了一个 Resource struct，用以实现资源集合(cpu/mem/自定义)的 大于、小于、增减等处理
```go
type Resource struct {
	...
	// MaxTaskNum is only used by predicates; it should NOT be accounted in other operators, e.g. Add.
	MaxTaskNum int
}
```

以及MinDimensionResource 方法 , 比如r1.MinDimensionResource(r2) 表示
1. 如果r1.cpu > r2.cpu，则r1.cpu = r2.cpu
2. 如果r1.mem < r2.mem，则r1.mem 不用动


### 调度task 时

proportion.underUsedFns 计算富余资源：session 初始化的核心是 计算Queue的 deserved ，之后每次 调度task 时，对queue deserved 与 allocated 求差值即 得到富余的资源
