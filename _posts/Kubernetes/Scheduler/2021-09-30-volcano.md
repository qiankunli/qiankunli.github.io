...
SchedulerCache 将调度所需的数据缓存起来，并保持与apiserver 同步。
...
由于在分布式系统中很难保证信息的同步，因此调度器经常以某一时间点的集群快照进行调度；在每个**调度周期**都会创建一个Session对象，用来存储当前调度周期的所需的数据，例如，Cache 的一个快照。当前的调度器中仅创建了一个Session，并由一个调度线程执行。PS：在一个调度周期，基于snapshot 数据，找到当前资源可以运行的最高优先级的pod，优先调度。**全局决策**，也是批量调度的一个内涵。
...
在allocate.go 中，针对一个task 的调度，会先找到task 所属queue 富余的资源（包括cpu/memory/自定义），如果富余资源够 task 使用，则资源申请成功，开始进入预选优选环节。 
...
其它：在一个调度周期内，一个queue 不会用到超过 deserved 的资源。 多轮调度周期下， t1 时刻 queue.deserve 是一个值，因为有新的task等导致 t2 时刻queue.deserved 值变小，此刻，一个queue的 allocated 大于了 deserved。
