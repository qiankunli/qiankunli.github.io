...
3. deserved 需要计算，分为多次循环，每次循环尝试给 queue 一点资源，直到给queue 的资源满足request，这个queue的分配就结束。这一点资源=  weight * remaing，下文称weight remaing resoruce (wrr)
...
for{
    for 每个queue{
        ...
        1. 如果 deserved 大于 capability，则取  deserved/capability/request 较小值，够request 用了，即meet了，为该queue的 分配技术
...
一次循环给 queue 属于它weight 配额的资源，够了就算了，不够等别人的配额分完了再来一轮。哪怕资源足够，proportion 也只是给queue 分配刚好够 request的资源。

这其中定义了一个 Resource，用以实现资源集合(cpu/mem/自定义)的 大于、小于、增减等处理
```go
...
proportion.underUsedFns 计算富余资源：session 初始化时 计算完成Queue的 deserved 和 allocated。之后每次 调度task 时，对queue deserved 与 allocated 求差值即 得到富余的资源
