...
就好比以前的单机系统演化为现在的分布式系统，你就不得不搞一些zookeeper、kafka、监控系统等保证正确性，且不得不接受单个请求 响应时间延长的问题，以确保整体吞吐量的增加。
...
如何提到面试“看人”的准确率？我们需要什么样的人？如何判断一个人是不是那样的人？
...
沟通很重要，第一优先是质量，但这个比较难，需要你跟小伙伴在一个“思维”上。如果不能保证质量，就要保证数量，周报不行改日报，一周一次code review 改成每日一次。你必须要承认一件事，如果小伙伴跟不上的话，手把手教其实更辛苦。其实手把手教还不是最辛苦的，这个时候尤其要注意，他是在努力踉跄的跟上，还是干脆等你拉着跑。
...
一个小队长的产出物（以此来安排自己每天的工作）

1. 一个明确的项目设计文档，一个清晰的脑图、架构图
2. 可能页面设计与数据库表设计
3. codere view结果
