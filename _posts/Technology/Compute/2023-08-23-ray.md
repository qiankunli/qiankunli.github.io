...
Ray 是一个并行和分布式 Python 的开源库。其定位是一个通用的分布式编程框架，能帮助用户将自己的程序快速分布式化。从高层次上看，Ray 生态系统由三部分组成：。Ray Core 提供了 low level 的分布式语法，如 remote func、remote class； Ray AIR 提供了 AI 场景的相关库（包括原生库和第三方库），以及用于在任何集群或云提供商上启动集群的工具。
...
2. Ray AIR是Ray的上层API，主要将Ray的各种应用整合在一起。比如 xgboost-ray
