...
**非常类似于 tensorflow 中的dataset**。分布式计算的精髓，在于如何把抽象的计算流图，转化为实实在在的分布式计算任务，然后以并行计算的方式交付执行。
...
在这样的编程模型下，Spark 在运行时的计算被划分为两个环节。PS： 与tf的dag 也很像，采用Lazy evaluation（对应Eager evaluation）
