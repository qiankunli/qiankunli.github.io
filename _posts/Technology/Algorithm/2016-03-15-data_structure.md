...
从解空间的视角看：
1. **一般需要暴力法先列出所有解空间，解空间的描述很多样**：窗口、树、矩阵（乘水最多的容器）等。
...
2. 暴力法之所以有问题，一方面是因为它产生重复的计算；另一方面暴力法往往还会去执行一些根本不可能是结果的代码。可以通过辅助数据结构、剪枝、滑动窗口来优化。注意，**你要砍掉的是不可能是结果的遍历，而不是想办法去拼凑解，因为你总是很难罗列所有情况**。

整体脉络：**构建解空间 ==> 筛掉不可能的解 ==> 确定遍历起点、方向 ==> 遍历某个点后看是否可以筛掉其它不可能的点、辅助计算其他点 以减少遍历**。
