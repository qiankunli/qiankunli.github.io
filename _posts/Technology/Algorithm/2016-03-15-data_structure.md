...
1. DFS 沿着树的深度遍历树的节点，尽可能深的搜索树的分⽀。 当节点 v 的所在边都⼰被探寻过，搜索将回溯到发现节点 v 的那条边的起 始节点。这⼀过程⼀直进⾏到已发现从源节点可达的所有节点为⽌。如果 还存在未被发现的节点，则选择其中⼀个作为源节点并重复以上过程，整 个进程反复进⾏直到所有节点都被访问为⽌，属于**盲⽬搜索**。
    ...
    3. DFS 是树的从上到下，还有从下到上（后序遍历，要借助辅助变量，比如二叉树的最大路径和问题），从左到右遍历。 
2.  BFS细分为带层的和不带层的。
...
3. 很多思路就是从暴力法来的，从笨方法优化而来，这是最稳妥的。你直接想一个精巧的方法， 很可能不符合所有情况。PS：这也是《算法通关之路》不厌其烦的每道题先讲一下暴力法。
2. 暴力法之所以有问题，一方面是因为它产生重复的计算；另一方面暴力法往往还会去执行一些根本不可能是结果的代码。可以通过辅助数据结构、剪枝、滑动窗口来优化。
