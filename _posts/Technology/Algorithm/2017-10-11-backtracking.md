...
2. 最优⼦结构决定了具体如何解决。DP 数组也可以叫”子问题数组”。**一个子问题的解要能通过其他子问题的解求出**，在整个问题的每一个子问题得到的答案都是最优的。一些问题 无法建立 f(n) 与 f(1) 到f(n-1) 关系，即**子问题之间无法建立关系**，此时可能是因为子问题的定义有问题（比如约束条件不够），或该问题无法使用动态规划。
