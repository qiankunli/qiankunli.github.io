...
dfs 带不带返回值，有两种习惯
1. dfs 只负责遍历，中间状态的 变化（比如path 新增元素与删除）代表了遍历的路径， 基本逻辑：状态扩充 + dfs 下探 + 状态回退，然后考虑 在回溯的模板上 加上 对当前节点/当前已遍历路径 的处理。
2. 通过返回值 感知到 子节点/子问题的结果，前提是 子节点根据 递归传递的信息 可以判断出子问题的结果。
