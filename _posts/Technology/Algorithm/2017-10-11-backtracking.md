...
3. 动态规划时间复杂度经常无法优化了（毕竟遍历dp是省不了的），有时候可以从状态数组 dp入手 优化空间复杂度，比如将二维dp 优化为一维dp（比如01背包问题）。从另一个角度说，用一维dp解决的也可以用二维dp解决，只不过很多二维`dp[i][j]` 的”网格“不具有合理含义，或没有遍历的必要，比如一些子序列问题dp[i] 等同于dp[0][i]。
...
如果是⼀维状态，那么我们使⽤⼀层循环可以搞定。如果是两维状态，那么我们使⽤两层循环可以搞定。PS：状态转移方程不一定都是`dp[i]`和`dp[i-1]`的方程（实质跟枚举类似），尤其是对于一些不连续的问题，`dp[i]`和`dp[i-1]`不一定能建立直接的推导关系，有时候`dp[i] = max(dp[0]..dp[i-1])` 等等
