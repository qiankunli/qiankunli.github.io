...
什么样的编程方式可以实现树节点和边的操作（遍历、查询）？递归。但**我希望大家换个方式来理解这个逻辑链条**：按照最笨的编程方法去操作树，发现需要下探和回溯，用到了栈。把函数的嵌套调用，看作访问下一个连通的结点；把函数的返回，看作没有更多新的结点需要访问，回溯到上一个结点。 **递归可以看做是这种通用逻辑的抽象、语法糖**
