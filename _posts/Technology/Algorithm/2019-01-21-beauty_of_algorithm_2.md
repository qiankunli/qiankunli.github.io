...
先将问题抽象为一个棵树，回溯法类似于深度优先遍历，动态规划类似于广度优先遍历。


贪心、回溯、动态规划适合解决的问题的模型：多阶段决策最优解模型。解决问题的过程，需要经历多个决策阶段。**每个决策阶段都对应着一组状态**。然后我们寻找一组决策序列，经过这组决策序列，能够产生最终期望求解的最优值。（树的节点不只是一个简单的数字，还可能是一个状态表示）
