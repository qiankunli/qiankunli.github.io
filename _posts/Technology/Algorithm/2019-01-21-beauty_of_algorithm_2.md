...
图和和树都有深度优先和广度优先遍历算法， 而很多问题都可以建模为树和图，所以你不仅要把问题的存储建模为树和图，还要将问题的解决建模为深度或广度遍历。树和图都有最优化问题，而最优化问题 **求解的过程** 类似一个树。从下文两位大牛的回答可以看到，动态规划和Dijkstra 都是广度遍历的推广。由此可以看到 **很多问题都可以建模为树和图（存储部分），解决过程可以建模为BFS或DFS，或者是对BFS或DFS的变种**。
