...
图和和树都有深度优先和广度优先遍历算法， 而很多问题都可以建模为树和图，所以你不仅要把问题的存储建模为树和图，还要将**问题的解决过程**建模为深度或广度遍历。树和图都有最优化问题，而最优化问题 **求解的过程** 类似一个树。从下文两位大牛的回答可以看到，动态规划和Dijkstra 都是广度遍历的推广，A*搜索算法又是对Dijkstra的优化。由此可以看到 **很多问题都可以建模为树和图（存储部分），解决过程可以建模为BFS或DFS，或者是对BFS或DFS的变种**。
...
4. 如何利用朴素贝叶斯算法过滤垃圾短信？可以认为数学原理的工程化运用了。
