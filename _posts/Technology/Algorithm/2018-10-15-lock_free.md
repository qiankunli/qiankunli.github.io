...
PS：单条指令的执行不是原子的，分为取地址、读数据、计算、存数据等多个步骤，且还要考虑cache的问题，但对单个Core来说是原子的，os或硬件保证在Core执行完一个指令后再响应中断。但是对多个Core来说，单条指令的执行（针对同一个数据）是可能会彼此干扰的。 MESI 保证Core间读写并发受控 ==> `LOCK cmpxchg`保证Core间写写并发受控 ==> 8字节的数据并发读写、写写安全 ==> 锁落实到底层是CPU中的锁指令。
