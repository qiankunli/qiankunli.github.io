...
1. 一个函数中如果出现了 yield 语句，那么它的函数标志 CO_GENERATOR(0x20) 就会被置位，当虚拟机在执行一个函数/CALL_FUNCTION的时候，发现函数的这个标志位被置位了，那就应该创建一个 Generator，而不是直接执行。PS：**以为 foo() 返回的是一个或多个 数字，实际返回的是一个Generator，foo 成了Generator 的一部分，在Generator.__next__ 执行时被触发**。
