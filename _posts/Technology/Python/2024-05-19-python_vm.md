...
[Python 函数在底层长什么样子？](https://mp.weixin.qq.com/s/ZEax7w4-q8GbesdVJAG5Vg)Python 一切皆对象，函数也不例外，函数这种抽象机制在底层是通过 PyFunctionObject 结构体实现的。PS：python代码很多动作都会对应一个c++对象， `def func` 就是new 一个PyFunctionObject，class 就是new 一个kclass/object 等？
