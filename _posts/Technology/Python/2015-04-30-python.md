...
2. 列表 list是动态的， 中括号，可以append和remove。
...
3. 经常看到或编写以下代码，除了函数名是“main”之外，这段代码与我们前面介绍的main函数没有半点关系，这个函数既不是必须的，也不能确定程序的执行顺序。他们之所以要编写`__name__ =='__main__'`，可能是因为想表明`main()`只在直接执行当前脚本时才运行，而在将其导入到其他模块时不要运行。
    ```python
    # main file
    def main():
        ……
