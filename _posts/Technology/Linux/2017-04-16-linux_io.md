...
|状态|用户态|内核态|用户态|
|---|---|---|---|
|代码|用户写入的程序代码，触发了调用|相关系统调用处理程序|用户写入的程序代码返回，继续执行|
...
3. 因此将**多个连接**第一部分的判断集中交给select系统调用。

也就是说，原先一个系统调用可以完成的事情，现在交给了两个系统调用，触发这两个系统调用的可能不是一个线程。

|io编程中线程理想状态|问题|io模型|适用|
|---|---|---|---|
|在一直处理数据|上层应用和网卡本质上是一个消费-生产关系，等待是免不了的|非阻塞，io复用或异步，socket与线程是多对一关系|io数据频繁，但单次数据较少|
