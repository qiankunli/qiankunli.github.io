...
**在 Linux 内核中，其实并没有线程的概念**。我们编程中所用的线程都是在用户态中申请内存，然后调用 clone 系统调用来创建的。内核原生的 clone 系统调用仅仅只是支持生成一个和父进程共享地址空间等资源的轻量级进程而已。
...
2. 线程创建 pthread_create ==> do_clone ==> clone ==> do_fork ==> copy_process。
    1. Linux 线程是包含了两部分的实现。第一部分是用户态的 glibc 库。我们创建线程调用的 pthread_create 就是在 glibc 库实现的。注意，glibc 库完全是在用户态运行的，并非内核源码，每一个线程对应一个struct pthread，存储了线程的相关信息，包括线程栈(线程没有办法使用操作系统默认给进程分配的栈内存，Linux 中 glibc 库的做法是自己申请内存来当线程栈用)。第二部分是内核态的 clone 系统调用。
