...
**mysql 作为一个数据库，其实就是sql的 解释执行器，这一点和jvm 作为字节码的解释执行器是一样一样的。**但跟java语言层面的并发安全又有所不同，java语言层面就两个安全级别：安全，不安全。而数据库提出隔离性的概念，并将隔离性分级，一般mysql 引擎仅实现部分并发安全。除此之外，除非隔离性要求很高，否则尽量避免动用锁。
