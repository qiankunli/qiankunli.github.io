...
2. 与hadoop2.x 类似，spark 也分为资源管理 + 任务调度
	...
	||master|slave|
	|---|---|---|
	|Hadoop2.x|ResourceManager|NodeManager|
	|mesos|master|salve|
	|spark|master|slave|
...
话说回来，Stream 和 rdd 本质都是 Builder 模式。在build 模式中，调用setxxx 都是铺垫，最后`build()`才是来真的。但Builder 模式带上更高维度的抽象，加上函数式编程（setxxx 时可传入方法）的助力，便真让人眼前一亮了。
