...
2. API / SDK；由 OTel 官方统一定义标准（跨语言一致）。push 模式，主动推送数据给 OTel Collector。
3. 接收、转换和导出遥测数据的工具，又称为 OpenTelemetry Collector。由 OTel 官方统一实现，可插件化扩展
4. OTel Backend，不绑定存储和展示方式，各厂商自定义或扩展 OTel 协议支持
