...
不准确的说，mesos调度容器，应该是mesos-master 向mesos-slave 发送命令，mesos-slave 执行`docker run xxx` 来运行容器。k8s 则直接促使其它厂商支持其定义的cri 接口，从` rpc RunPodSandbox(RunPodSandboxRequest) returns (RunPodSandboxResponse) {}`之类的接口可以看到，要求支持cri的厂商支持自家的pod概念

	![](/public/upload/kubernetes/cri.png)
	
### cni

这块参见[Docker网络五,docker网络的回顾](http://qiankunli.github.io/2016/10/12/docker_network5.html)，要点是：

1. cni的原理，不赘述
2. 为容器分配ip，保持容器互通，并不是k8s/cni 介入网络的必要理由，因为docker也可以做到。它是一套组合拳，k8s中有namespace以及network-policy的概念，即想自由控制一个容器集群的不同namespace的容器是否互通。

### csi

笔者理解较浅显

实现回顾下volume，来一段精彩的描述：[Container Storage Architectures: How Does Kubernetes, Docker, and Mesos Compare?](https://blog.thecodeteam.com/2017/06/27/container-storage-architectures-kubernetes-docker-mesos-compare/)几个要： Containers promise application portability far beyond what virtualization can achieve because the hypervisor has requirements for hardware emulation（相比hypervisor，Containers极大的提高了application的可移植性）. The ability to achieve application portability is going to rely on interoperability amongst container orchestrators. Even with modern cloud-native applications, storage is a critical component because applications can take advantage of persistent storage platforms and develop around some of its features. See Clint Kitson’s blog [Understanding Storage in a Cloud Native Context for more information](https://blog.thecodeteam.com/2017/05/16/understanding-storage-cloud-native-context/).


1. docker volume plugin 的核心原理：If a plugin registers itself as a VolumeDriver when activated, it must provide the Docker Daemon with writeable paths on the host filesystem
2. 使用k8s csi 之后，可以做到：
    
    1. 不同的服务可以挂载不同的volume plugin，由配置指定
    2. 一个服务重启时，虽然运行主机变了，但仍可以挂载原先的volume，这对部署有状态服务意义重大。
3. docker 只能使用本地volume，volumn 不能随主机移动。估计这也是，csi 提高到编排层面的好处所在。


本地volume 挂载全局分布式文件系统来实现类似的效果。比如lizardfs，每个host 装一个lizardfs client，将lizarddfs root 目录挂到本地的`/mnt/docker-net-volume`，运行容器时 `docker run --it -v /mnt/docker-net-volume/xx:/containerDir imageName command` 但缺点是：

1. 要求所有的dfs 都具备这样的client
2. `docker run --it -v /mnt/docker-net-volume/xx:/containerDir imageName command` 中的`/mnt/docker-net-volume/xx ` 对于docker 来说，还是以本地目录的角度来使用的volume的。docker 本地处理的问题在于：单单依赖docker， volume 无法纳入管理，更具体的说，docker不知道什么时候销毁volume。

基于csi，一个k8s的例子，来自[CSI - Container Storage Interface（容器存储接口）](https://jimmysong.io/kubernetes-handbook/concepts/csi.html)
...
根据k8s api，即可以自由控制 my-request-for-storage volume和my-frontend container 的生命周期。
...
3. 此时，docker仍然掌控着全局，虽说plugin 的部署在docker 之外。
4. k8s 通过cri、cni、csi三大接口，docker 退化成了一个cri实现（也就是cpu与内存资源的封装）。**cni、csi的意义不仅在于实现了跨集群的连通性 和 共享存储，更在于 在集群维度管理 它们 更有价值**，这也是`docker
 network` 和 `docker volume` 一直在努力的目标。
...
归根到底，mesos还只是docker的使用者，也必然是docker特性的迁就者。而k8s则试图将一切掌握在自己手中，只给docker留一个cri的位置。比如，正是csi的提出，k8s才具备条件管理有状态服务（**k8s的大部分特性，不是说原来的docker或者后来的mesos做不到，而是k8s将这一切规范了、可配置、可管理了**）。
