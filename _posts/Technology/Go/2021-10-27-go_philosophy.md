...
Go 语言之父 Rob Pike 说过“接口越大，抽象程度越弱”。越偏向业务层，抽象难度就越高。所以Go 标准库小接口（1~3 个方法）占比略高于 Docker 和 Kubernetes 的原因。**Go 接口是构建 Go 应用骨架（对应血肉）的重要元素**。抽象的时机：在实际真正需要的时候才对程序进行抽象。再通俗一些来讲，就是不要为了抽象而抽象。接口的确可以实现解耦，但它也会引入“抽象”的副作用，或者说接口这种抽象也不是免费的，是有成本的，除了会造成运行效率的下降之外，也会影响代码的可读性。PS：**接口由使用方按需定义，而不用事前规划**。彻底解耦了模块的生产者和消费者。业务方写业务方的代码，架构方写架构方的逻辑，双方通过一个抽象的协议达成默契，没有任何硬编码的层级绑定。这就是大型软件工程里梦寐求的真正的解耦。
...
1. 完全面向对象设计就是一刀切的宗教式的设计，但其并不能很好的表述这个世界，这就导致其表现力不足，最后通过设计模式和面向切面等设计技巧来弥补语言方面的缺陷； JAVA就好比：手里握着是锤子，看什么都是钉子，什么都是类的对象，这个和现实世界不符，类表示单个事物还可以，一旦表示多个事物及其交互，其表现力也就会遇到各种挑战。类型是天然的，把类型以及对类型的操作封装在一起也是合理的。数据结构加上操作这些数据结构的函数，本来就是程序运行的客观规律。但是，面向对象编程错就错在，**它非要把这种天然的数据与行为的封装，强行绑定在一个严格的父子层级关系树上*，并试图用这种静态的分类学来描述不断演化的动态业务世界。
	1. 第一个原罪，脆弱的基类。在深层继承树中，基类承载了太多它本不该承载的责任。基类的一点点微小改动，都会像休克波一样顺着继承树向下层层传递，影响成百上千个子类。你以为你只是在最顶层的父类里加了一个无害的初始化日志方法，结果可能导致几百个子类在反序列化时的内存占用瞬间飙升。
	2. 第二堆内存严重碎片化导致的CPU缓存未命中灾难。现代CPU的运行速度是非常恐怖的，但它的瓶颈在于从主内存中读取数据的延迟。为了缓解这个延迟，CPU内部设计了L1、L2、L3等多级高速缓存。如果你的数据结构在内存里是连续且紧凑的，CPU读一次内存，就能把后续要处理的几十个节点数据全部顺便加载到L1缓存里。接下来的几十次循环计算，CPU都不需要再去主内存里拿数据，缓存命中率高，整体的计算性能就能直接起飞。但是，因为面向对象那种分散的堆内存分配模式，再加上为了多态而引入的指针跳转，导致CPU在遍历节点时，不得不在庞大的内存地址空间里疯狂地来回跳跃去抓取数据。CPU刚刚加载好的缓存行立马就失效了，不得不在极长的时间里处于等待主内存响应的闲置状态，这在底层性能优化领域被称为**指针追逐问题**。在Rust里，因为没有了继承的羁绊，结构体的内存布局是扁平、透明且完全可预测的。你可以非常轻松地在栈上分配紧凑的数据，或者创建连续的结构体数组。Rust这种对缓存极其友好的紧凑内存布局，加上它独树一帜的编译期单态化处理，让计算性能达到了极致。Rust的单态化机制采取的是一种空间换时间的强硬策略。在编译期，当Rust的编译器发现你在代码里使用泛型和Trait进行多态调用时，它根本不会在运行时搞什么查表操作。相反，它会找出你实际传入的所有具体类型，然后为每一种具体类型，单独生成一份专门的机器码副本。所有的抽象和多态都已经被编译器彻底抹平了，所有的函数调用都是最直接、最快速的静态方法调用。
