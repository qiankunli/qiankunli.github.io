...
4. mcentral：收集了给定大小等级的所有 span，对应于 TCMalloc 中的 Central cache 结构。
5. mcache：为 per-P 的缓存。对应于 TCMalloc 中的 Thread cache 结构。
...
1. 在 Go 的三级内存管理器中，维护的对象都是小于 32KB 的小对象。对于这些小对象，Go 又将其按照大小分成了 67 个类别，称为 spanClass。每一个 spanClass 都用来存储固定大小的对象。
