...
[Go 语言网络库 getty 的那些事](https://mp.weixin.qq.com/s/hihY7sEatJCHTTZHs3urfQ) 
...
|分层|接口|用户需要做的工作|
|---|---|---|
|EventListener|onOpen/onError/onClose/onMessage/onCron| 在onMessage 中写入业务逻辑|
|数据交互层|ReadWriter | 机型byte[] 和 Message 的转换|
...
session 会持有buffer、channel 等用于 工作流中 数据暂存、协程间沟通
```go
