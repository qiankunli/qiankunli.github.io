...
Netty中的所有的IO操作都是异步的，而不是像传统BIO那样同步等待操作完成。同步操作的行为是可以预期的

1. 要么就等着操作完成
2. 可以设置timeout

但异步操作带来一个问题：调用者如何获取异步操作的结果？

异步指的是，调用者向执行部件发出一个调用，不等执行部件返回结果，继续向下执行。执行部件有三种途径向调用者返回状态

1. 状态，调用者每隔一段时间检查下执行部件的状态
2. 通知，执行部件保有调用者引用，执行完毕后，执行调用者的相关方法
2. 回调，跟通知没多大区别，调用者和执行部件共同保有（Future作为共同参数或者被执行部件的异步方法返回）一个Future（存储结果），执行部件执行Future的setter方法，调用者执行Future的getter方法。大部分时候，调用者和执行部件身处两个线程，所以Future要线程安全。

事实上，Netty Future的建议操作模式就是赤裸裸的通知，执行部件改变状态时，会执行注册在future上的Listener（变相的观察者模式）。

Promise是可写的Future，提供写操作相关的接口，用于设置IO操作的结果。比如DefaultPromise的setSuccess方法
