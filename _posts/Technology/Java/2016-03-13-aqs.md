...
细心的读者或许发现了，mutex的lock方法使用的是`sync.acquire(1)`，而不是`sync.tryAcquire(1)`。因为tryAcquire仅仅是尝试改变状态值，如果改变不成功就返回false，让调用者看着办。在`sync.acquire(1)`中，会先调用`sync.tryAcquire(1)`，如果设置状态值失败，会将当前线程加入阻塞队列。
...
1. 自动管理这个同步状态state（int类型），更新的时候需要用CAS保证原子性。内置锁是Object在堆中内存的一个标识位，访问唯一性通过字节码指令实现。
2. 阻塞和唤醒线程park/unpark。Object内置锁的阻塞和唤醒要通过操作系统（java线程用的是操作系统线程）。
3. 队列管理，一个双向链表实现queue。这样一次可以决定唤醒一个或多个线程，而Object的内置锁要么notify随机一个要么notifyAll。
