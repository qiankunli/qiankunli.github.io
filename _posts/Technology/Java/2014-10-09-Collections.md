...
不管容器类底层什么实现，其实就三种，数组，链表和树。进而，容器类每次add时，必须要有机制确定数据存放在哪个位置（根据加入的先后（list），hashcode方式（hashset，hashmap），tree则根据元素本身的大小（treeset和treemap），**我觉得背概念害死人，以这个角度来划分容器类，更顺一些**）。从另外一个维度讲，collection存储的基本单位是元素，map存储的基本单位是键值对。
