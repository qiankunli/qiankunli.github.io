...
为什么要有set？一个可能的原因是：“触发那些监听结果的listener的执行“要有一个统一的执行位置，比如NettyPromise的实现
...
而有些框架（主要是io框架）的Future实现中，**Future只是两个线程间共享数据的载体**（future简化了io线程与业务线程的并发访问），包含线程安全的设置与获取结果的方法（或者说，只是对一个Object或变量的封装，使其安全访问），本身不是FutureTask，不包含执行逻辑。

**因为future与执行逻辑的分离**，在io框架中，我们通常需要建立future与业务收发数据（以下称为message）的关联。比如`Future send(Message)`，其实现通常是

	// 一般会有一个定时线程，对过期的callback数据进行清理
	map<long,future> callback
	Future send(Message){
		Future future = new Future();
		callback.put(message.getId(),future);
		ioBusiness.send(message);
	}
	void receive(message){
		// 收发的同一个message必须具备相同的id
		Future future = callback.get(message.getId());
		future.setResult();
	}
