...
从数学形象上看，编程是一个收敛过程。需求的生发是发散的——用户什么都想要，什么都觉得重要。需求的确认是收敛——哪些是核心场景，哪些是边缘情况，哪些留到下一版。技术调研是发散的——查文档、看方案、读源码。技术选型是收敛——选一个方向，放弃其他。设计是发散的，方案评审是收敛的。 计划是发散的，任务编排是收敛的。 编码是发散的，测试是收敛的。测试结果出来，有 bug，编码再发散，修完再收敛。一轮又一轮，直到系统收敛到一个满足需求的状态。一个好的编程过程，是让收敛高效发生。一个坏的编程过程，是让发散持续失控。
...
比如MyBatis、Spring、SpringMVC、HttpClient等，它们并没有在一个大类中实现各种各样的功能，而是切分放在不同的类中，并且通过多层继承关系组合在一起，不管是可读性上，还是可扩展性上都非常不错。
