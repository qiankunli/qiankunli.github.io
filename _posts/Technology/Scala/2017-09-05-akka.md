...
2018.4.3 补充 [Fork and Join: Java Can Excel at Painless Parallel Programming Too!](http://www.oracle.com/technetwork/articles/java/fork-join-422606.html) 详细解释了引入 fork join的初衷，要点如下：（未完成）
...
3. fork/join 为何如此有意义
	...
	* concurrency（并发） 和 parallelism（并行）. 现在的多核cpu，并发是不够的，要并行。
