...
2. 服务端响应 syn 请求的主要工作是 判断接收队列是否满了，满的话可能直接丢弃该请求，否则发出synack，申请request_sock 添加到半连接队列中，同时启动定时器。
3. 客户端收到服务端的synack 时清除connect 时设置的重传定时器，把当前socket 状态设置为established，开启保活计时器后发出第三次握手的ack 确认。 
