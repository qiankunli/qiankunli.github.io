...
2. 记录的隐藏字段
    1. DB_TRX_ID	记录操作该数据事务的事务ID；
    2. DB_ROLL_PTR	指向上一个版本数据在undo log 里的位置指针；
    3. DB_ROW_ID:	隐藏ID ，当创建表没有合适的索引作为聚集索引时，会用该隐藏ID创建聚集索引;
3. 在innodb 中每个事务开启后都会得到一个read_view。主要保存了当前数据库系统中正处于活跃（没有commit）的事务的ID号，换句话说，保存的是系统中当前不应该被本事务看到的其他事务id列表。
    1. 当数据的事务ID不满足read view条件时候，从undo log里面获取数据的历史版本，然后数据历史版本事务号回头再来和read view 条件匹配 ，直到找到一条满足条件的历史数据，或者找不到则返回空结果；

### 工作原理（不同文章不同）

基于Timestamp的Scheduler会在事务开始时候分配一个全局自增的Timestamp，这个Timestamp通常由物理时间戳或系统维护的自增id产生，用于区分事务开始的先后。同时，**每个数据库对象需要增加一些额外的信息**（多像jvm 对象的mark word 的线程id），这些信息会由对应的事务在访问后更新，包括:
