...
MVCC就是通过“读取旧版本数据”来降低并发事务的锁冲突，提高任务的并发度。旧版本数据存储在哪里？存储在回滚段里（即undo日志），回滚段里的数据，其实是历史数据的快照（snapshot），这些数据是不会被修改，select可以肆无忌惮的并发读取它们。普通select都是快照读，快照读不加锁，事务读取到的数据要么是事务开始前就已经存在的数据，要么是事务自身插入或者修改的数据。
