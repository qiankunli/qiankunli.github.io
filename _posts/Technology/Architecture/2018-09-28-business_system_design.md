...
[一文教你认清领域模型和数据模型](https://mp.weixin.qq.com/s/DIZuKHiNbH21hNiRYIVNEQ)领域模型是面向领域对象的，要尽量具体，尽量语明确，显性化的表达业务语义是其首要任务，扩展性是其次。而数据模型是面向数据存储的，要尽量可扩展。PS：领域模型跟技术毫无关系，而是为了更有**结构化的拆解和表达业务逻辑**。
