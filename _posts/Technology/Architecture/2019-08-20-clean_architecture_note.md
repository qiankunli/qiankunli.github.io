...
2. 而细节是指你这些让操作系统的人、其它系统以及程序员们与策略进行交互，但是又不会影响到策略本身的行为。它们包括IO设备、数据库、web系统、服务器、框架、交互协议等。
...
假设硬件不存在会怎么样？在如今这可不算开玩笑了。我们还需要按文件、表组织数据么？当然不，我们会将数据存储为链表、树、哈希表、堆栈、队列等各种数据结构，然后用指针或引用来访问这些数据。
