...
工程的本质就在于，在各种静态与动态约束之间，构建尽可能合理的解决方案，而非追求完美。
1. Martin Fowler 认为软件架构是：重要并且难以改变的决策。
2. 铁原：架构，为复杂系统提供确定性和可能性空间。
3. 黄归速：架构的组成 = 要素 + 结构 + 连接，将系统要素按照特定结构进行连接交互。软件架构指软件系统顶层结构设计。架构是经过系统性地思考，权衡利弊之后在现有资源约束下的最合理决策，最终明确的系统骨架：包括子系统、模块、组件，以及他们之间协作关系、约束规范、指导原则，并由它来指导系统各方面的设计和指导团队中的每个人思想层面上的一致。软件架构的设计无非就是前后端的双向打通，前端是面向用户的 user friendly 接口，考验的是程序员对业务的理解与设计；后端是实现功能所依赖的基础能力，考验的是程序员对底层技术的掌握与应用；而前后端的打通，考验的是程序员的能力、经验与审美。
4. 段张博：架构的本质，是在混沌中建立秩序。它回答一个根本问题：**海量代码与逻辑，如何安放**？架构负责塑造系统的结构与关系，让复杂变得可理解、可协作。架构同时也是一组关键决策，在业务、成本、时间等约束下做出深远且难以逆转的取舍。架构的本质目标就是管理复杂度。
...
![](/public/upload/architecture/biz_arch.png)

业务架构是战略，应用架构是战术，技术架构是装备。其中应用架构承上启下，一方面承接业务架构的落地，另一方面影响技术选型。熟悉业务，形成业务架构；根据业务架构作出相应的应用架构，最后技术架构落地实施。你也可以理解成：业务架构是生产力，应用架构是生产关系，技术架构是生产工具。业务架构决定应用架构，应用架构需要适配业务架构，并随着业务架构不断进化，同时应用架构依托技术架构最终落地。实际工作中，业务架构是其他架构设计的输入源头。例如业务架构中“风险控制能力”需要数据架构建立风控模型库，应用架构开发风控引擎服务，技术架构部署实时计算集群。PS：最好的技术设计，一定是最符合领域特征的设计。需求的目的，是让系统更好卖；设计的目的，是降低开发维护成本，快速响应细节的变化。组织的利润 = 需求 - 设计。先从业务角度出发，设计好做什么，每个模块职责划分，驱动力未来便于演变。再从技术角度出发，看每个模块怎么做，驱动力是核心矛盾（尤其是性能问题）。

| 维度       | 业务模块划分（做什么）                | 技术架构分层（怎么做）                  |
|------------|------------------------------|----------------------------------|
| 切分方向   | 在同一抽象层次上进行功能分割（水平切分）         | 在不同抽象层次上进行职责分离（垂直切分）            |
| 关注焦点   | 业务能力和领域边界,DDD           | 技术职责和依赖关系,单一职责；依赖倒置；接口隔离；开闭原则               |
| 设计目标   | 业务内聚、模块解耦           | 职责分离、层次清晰               |
| 变化驱动   | 业务需求变化                 | 技术架构演进                     |
| 团队视角   | 产品经理、业务分析师         | 架构师、技术负责人               |
| 生命周期   | 相对稳定，随业务发展调整     | 相对固定，技术架构确定后很少变化 |

在实际的企业级应用中，这两种架构思维是正交关系，需要同时应用：每个业务模块内部都包含完整的技术分层，这就是现代软件架构的二维设计空间。优秀的架构师不是选择其中一种思维，而是能够熟练地运用两种思维，在二维设计空间中找到最适合当前业务和技术环境的架构方案。
1. 业务/领域架构：商品、订单、库存、权益等如何划分和协作
2. 技术/系统架构：单体还是微服务、读写链路、存储和容灾策略
3. 实现/框架层：用什么框架、用哪些组件、如何接入治理体系。框架层只是前两层设计的实现手段。如果前两层是空的，只堆了一串TMF、BOS、Spring的名词，本质上是架构空心化。架构是关于边界和取舍的长期决策，框架只是实现这些决策的工具。说我们用了某某框架，所以有了某种架构，是把因果关系完全倒过来了。更麻烦的是，一旦把框架抽象当成领域抽象，就会出现几类典型坑：
    1. 业务实体被异化成各种通用BizObject、Task，看起来统一，实则一锅粥
    2. 以为打满扩展点就叫可扩展，但职责边界、治理规则都没想清楚
    3. 问题一多，大家只会说框架有坑、中间件不稳定，却说不出架构哪里设计错了

PS：横向可并列，纵向可推导，整体可演进。横向分层：明确系统的层次结构设计。纵向功能分解：系统各个层次包含的哪些应用服务。
...
我：一个良好的架构，新来一个需求，代码该在哪里加、改是显而易见的，如果不是显而易见的、或者改哪里都可以，那说明原先架构的设计没有体现这个概念，应该动架构。把代码比较均衡的摊在各个位置，不均衡一般意味着设计有问题。 
段张博：架构的本质，是在混沌中建立秩序。它回答一个根本问题：海量代码与逻辑，如何安放？
1. 架构关心的，是系统长什么样、怎么长大：拆成哪些领域和模块，各自负责什么；数据和职责怎么划分；在性能、稳定性、成本、交付之间做了哪些取舍；未来业务变化时怎么演化。这是一组长期有效、难以回滚的关键决策。
2. 框架关心的，是怎么高效把这些决策落到工程实现上：TMF、BOS、Spring提供的是编程模型、基础能力（RPC、配置、监控、灰度等）和扩展机制（扩展点、SPI）。它们解决的是少造轮子、接好基础设施的问题，而不是帮你定义领域边界和职责。