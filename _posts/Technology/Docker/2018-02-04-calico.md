...
iptables acl 根据 profile/policy allow 或者 drop 流量。

### 熟悉bgp

[calico初识](http://qiankunli.github.io/2018/02/04/calico.html)
...
1. Node-to-Node Mesh，默认策略，也叫full-mesh，节点与节点之间两两相联
2. global bgp peer。如果停止 Node-to-Node Mesh，并配置一/多个 global peer，Calico will peer every Calico node with these peers
3. node specific BGP peer


一个网络凭空多了许多vRouter，需要与其它vRouter 通过bgp 同步信息，

根据bgp peer 的有无以及类型的不同，有多种组网方式，都可以实现分发路由的效果，只是适应的网络规模、架构等有所不同[IP Interconnect Fabrics in Calico](https://docs.projectcalico.org/v2.6/reference/private-cloud/l3-interconnect-fabric#fn:1)
