...
因为Content Addressable Storage (CAS)，所以当你delete 某个镜像后，一样的内容再改个image名字 重新push 是没有用的（还是会拉不到image），因为Content Address 是一样的，Content Address 对应的manifest 还是被软删的状态 。所以笔者试了一下在项目中随便改动一些无关紧要的文件，Content变了，Content Address 也就变了，重新build and push，便可以抵消harbor delete 误操作带来的问题。
