...
使用docker后，物理机的角色弱化成了：单纯的提供计算资源。容器可以在物理机之间自由漂移，但许多系统的正常运行依赖ip，ip不稳定带来一系列的问题。而解决ip的变化问题主要有以下方案
...
ip的经常变化为开发童鞋的调试带来了很大不便。最初，我们提供了一个web console来访问容器，操作步骤为：login ==> find container ==> input console ==> op。但很多童鞋依然认为过于繁琐，并且web console的性能也不理想。而直接为每个容器配置ssh server，又会对safe shutdown等产生不良影响。因此
