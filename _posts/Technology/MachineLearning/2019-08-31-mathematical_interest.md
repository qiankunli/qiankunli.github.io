...
$$P(data|θ)P(θ)=θ^7(1−θ)^3P(θ)$$

则`P(data|θ)P(θ)`的函数图像为：
...
3. \\(w1=w1+\sigma d_{w1}\\) 依次得到 w1,w2,b 的新值
...
一般来讲，误差满足平均值\\(\mu\\)=0的正态分布 

$$f(x)=\frac{1}{\sqrt{2\pi}\sigma}e^-\frac{(x)^2}{2\sigma^2}$$

将\\(E^i\\)代入
 
 $$p(y^i|x^i;\theta)= f(E^i)= \frac{1}{\sqrt{2\pi}\sigma}e^-\frac{(y^i-h^i)^2}{2\sigma^2}$$
