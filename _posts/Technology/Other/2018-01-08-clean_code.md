...
1. 对于java，开发直接写的是代码，会被翻译成字节码。字节码分为指令和数据两个部分。
2. 对于数据库，开发直接写的是sql，会被解释为执行计划。数据库保证事务（sql序列）的ACID

它们的一个共同点是：虽然可以设置，但jvm不会直接保证一行java代码是原子的，数据库不会直接保证一个sql执行是原子的。而这些都是程序猿直接看到的。
