...
||接口|实现struct||
|---|---|---|---|
|应用层/连接层|ClientConnInterface|ClientConn|ClientConn represents a **virtual connection** to a conceptual endpoint, to perform RPCs|
|Stream层|ClientStream|clientStream|
|transport层|ClientTransport|http2Client|
|tcp层|net.Conn|
