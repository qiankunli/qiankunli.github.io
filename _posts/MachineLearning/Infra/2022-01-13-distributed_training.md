...
4. 在张量并行中，也称为层内模型并行，用于在多个GPU上训练LLMs。**将不同的算子拆分到不同的设备上**，每个设备执行算子的部分操作，最终聚合得到某个算子的计算结果。。与数据并行不同，张量并行通常需要高带宽连接进行通信，因此更常用于单个GPU节点内（多个gpu）。比如矩阵相乘就可以根据其数学性质进行拆分：$XA=\begin{bmatrix}X_1 & X_2\end{bmatrix}\begin{bmatrix}A_1\\A_2\end{bmatrix}=X_1A_1 + X_2A_2 = Y_1 + Y_2 = Y$
