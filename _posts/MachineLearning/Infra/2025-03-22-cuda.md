...
1. 入门版：仅描述数据块和计算逻辑，其余硬件实现细节和优化全部由编译器完成。hardware independent
2. 进阶版：在入门级的基础上，除了描述数据库和计算逻辑，可以调用一些封装的算子，兼顾开发效率和灵活性
3. 精细版：可以进行精细化的线程级开发，适用于极致性能优化场景。
同时也支持几种层级混合使用。
![](/public/upload/machine/tilelang.png)
