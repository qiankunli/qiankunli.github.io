...
PS：针对一个输入的 Prompt，通常是由一个 tokens 序列组成的，这里 Attention 输出的 out 张量，对于推理场景而言，这里自然关心的是最后一个 out 向量。最新 token 位置对应的 out 向量，依赖前面所有 token 的 K 向量，前面所有 token 的 V 向量，以及仅仅依赖当前 token 的 Q 向量。也回答了为何不缓存Q？Q 即用即算，没有缓存的意义。
