...
[[源码解析] PyTorch 分布式(8) -------- DistributedDataParallel 之 论文篇](https://mp.weixin.qq.com/s/5EL3yb_-8t02GLdZ6qsclw)PyTorch将值组织成Tensor，Tensor是具有丰富数据操作集的通用n维数组。module定义了从输入值到输出值的转换，其正向传递期间的行为由其 forward 成员函数指定。Tensor可以包含Tensor作为参数。例如，线性module包含权重参数和偏差参数，其forward函数通过将输入乘以权重并添加偏差来生成输出。

应用程序通过将本机module（如线性、卷积等）和自定义forward函数中的Function（如relu、pool等）粘合在一起，构成自己的模块。典型的训练迭代包括使用输入和标签生成损失的前向传递，计算参数梯度的后向传递，以及使用梯度更新参数的优化器步骤。更具体地说，在向前传播过程中，PyTorch构建了一个autograd图来记录所执行的动作。然后，在后向过程中，使用autograd图进行反向传播以生成梯度。最后，优化器应用梯度来更新参数。训练过程重复这三个步骤，直到模型收敛。
...
Q = {Tensor} 
 ...
 grad_fn = {SubBackward0}               # 每一个前向传播操作的函数都有与之对应的反向传播函数用来计算输入的各个 variable 的梯度，这些函数的函数名通常以Backward结尾
  metadata = {dict: 0} {}
