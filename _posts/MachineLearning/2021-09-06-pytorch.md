...
```
反向传播传播的啥？梯度。 以减法  `tmp = y_-y` tmp 的梯度 grad_output 通过 Sub.backward 方向传播 给`y_` 的是  `grad_output * 1`，给y 的是 `grad_output * -1`。对应到链式法则 就是 `dl/dy = dl/d_tmp * d_tmp/dy`。 可以对照 Sub/Mul/Div backward 方法找找感觉。
...
1. 每一个前向传播操作的函数都有与之对应的反向传播函数用来计算输入的各个 variable 的梯度，这些函数的函数名通常以Backward结尾。比如 加法对应 AddBackward。 在pytorch v0.1.0 中
   ...
   class Sub(Function):
        ...
        def backward(self, grad_output):
            return grad_output, grad_output.neg()
    ```
2. `torch.autograd.backward(z) == z.backward()` 是等价的，Pytorch中所有的计算其实都可以回归到Tensor上
