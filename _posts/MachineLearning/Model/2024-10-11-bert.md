...
1. GPT 自回归的设计（casual attention+next token prediction）能非常高效地利用预料中每一个 token，为 Transformer 提供训练梯度，BERT 的 “完形填空” 任务的预料利用效率可能只有不到 20%。
2. GPT 的设计，适合高效地对接所有的文本语料，进行数据 Scaling Up，构建世界模型。
3. 生成任务比理解更难，Scaling Limit 更高，并且通过 “生成” 带动了 “理解”，实现了生成和理解的统一和超越。
