...
$$ 
$$ 
= E_{(s_t, a_t) \sim \pi_{\theta'}} \left[ \frac{P_\theta(s_t \mid a_t)}{P_{\theta'}(s_t | a_t)} \frac{P_\theta(s_t)}{P_{\theta'}(s_t)} A^\theta(s_t, a_t) \nabla \log p_\theta(a_t^n \mid s_t^n) \right]
$$ 
$$ 
= E_{(s_t, a_t) \sim \pi_{\theta'}} \left[ \frac{P_\theta(s_t \mid a_t)}{P_{\theta'}(s_t \mid a_t)} A^\theta(s_t, a_t) \nabla \log p_\theta(a_t^n \mid s_t^n) \right] \quad \text{\# 假设 } s_t \text{ 不受到 } \theta \text{ 影响}
$$ 

因此我们能得到一个新的Objective Function：
$$ 
J^{\theta^k} (\theta) = E_{(s_t, a_t) \sim \pi_{\theta^k}} \left[ \frac{P_\theta(s_t \mid a_t)}{P_{\theta^k}(s_t \mid a_t)} A^{\theta^k} (s_t, a_t) \right] \quad \text{\# Note: } \nabla f(x) = f(x) \nabla \log f(x)
$$ 
...
$$ 
J^{PPO2}_\theta (\theta) \approx \sum_{(s_t, a_t)} \min ( \frac{p_\theta(a_t \mid s_t)}{p_{\theta^k}(a_t \mid s_t)} A^{\theta^k} (s_t, a_t), \text{clip} ( \frac{p_\theta(a_t \mid s_t)}{p_{\theta^k}(a_t \mid s_t)}, 1 - \varepsilon, 1 + \varepsilon ) A^{\theta^k} (s_t, a_t))
$$ 
