...
如果你想先看通用 RL 主线，可以先读[从长期回报、Credit Assignment 到 PPO]({% post_url 2026-04-06-rl-mainline %})；如果你想看不局限于 LLM 的策略梯度、Actor-Critic、GAE、PPO 细节，可以再看[强化学习细节：从机器人行走到 PPO]({% post_url 2025-02-18-rl %})。这篇则专门聚焦在 **RL 放到 LLM post-train 语境后，状态、动作、奖励、Critic、KL、GRPO 都分别对应什么**。
...
GRPO所有 token 都被平均奖励或惩罚，太粗糙，比如在文本生成里，前几个 token 决定了主题走向，后面只是延续，如果整段输出 reward 高，那么早期 token 的功劳更大，但 GRPO 一视同仁。GSPO 想解决的就是这个问题：让advantage细化到每个 action/token/step，让每个token都有不同的强化强度。

$$
A_t^{(i)} = w_t^{(i)} \cdot \left( r^{(i)} - \bar{r}_{\text{group}} \right)
$$

这里$w_t^{(i)}$就是 step-wise 权重，可以基于多种策略计算 (如 log prob、位置（靠前的 token 权重更高）、attention 等)。从而在长序列生成或多步推理任务中，提升收敛效率与稳定性。
