...
LLMEngine.step  运行一次推理，此处有4个层级的概念 LLMEngine/Executor/Worker/ModelRunner，层层分解，各管一摊。
...
class LLMEngine:
    def step(self) -> ...:
        ...
        return request_outputs    
```
Executor 作为 LLMEngine与Worker 中间层，干的活儿不多，主要是中转，主要是抽象 单卡和分布式执行。GPUExecutor.execute_model ==> Worker.execute_model
```python
...
class Worker(WorkerBase):
    ...
    def execute_model(self,execute_model_req: Optional[ExecuteModelRequest] = None) -> List[Union[SamplerOutput, PoolerOutput]]:
        ...
        return [output]
```
ModelRunner 是真正干推理的。Attention计算、compute_logits 接口和sample。
```python
class ModelRunner:
...
### cacheManager（还有很多不清晰的地方）

kv cache是通过Worker 中类型为CacheEngine的一个数据成员allocate出来的。所谓申请gpu内存，就是`torch.zeros(device='cudda')`
...
PS：几个疑问
1. 模型执行层，只管读取kvcache，Attention模块基于kv cache 生成新的token 如何加入到kvcache 由上层负责？
2. cacheManager 一次申请了多少block？物理block 的管理和_allocate_kv_cache 有啥关系？
3. LLMEngine 在init 时便_initialize_kv_caches 申请了一定数量的gpu block。后续不用再申请了？每个seq 不仅存了当前的token list，还存了对应的kv cache（token 对应的kv cache tensors），那到model.forward 这个seq的时候，要么model.forward 用的kvcache 应用指向这个seq的kvcache，要么把这个seq 的kvcache copy到model.forward 指向的kvcache 上（如果model.forward 真实用到的kvcache 物理块必须是连续的话）。vllm 修改了 FasterTransformer 中的注意力内核，根据块表读取 KV 缓存，并即时执行注意力操作。
几个猜测
1. seq 持有的block 不仅存tokenid 也存token 对应的kv cache tensors。毕竟只存tokenid 不需要多大空间。
2. 每个seq 在推理之前，即预留好了新token 以及对应的kv cache tensors的空间。
3. 对seq 进行 model.forward时，需指定一个kvcache 引用，此时会将这个seq 对应的kv cache tensor引用给它。但不是直接给，vllm 应该是改了底层机制，让model.forward 读写kvcache 刚好都是seq的kv cache显存空间，且应用层看起来是顺序的，实际是离散的。新的tokenid 应用层返回，新tokenid对应的kv cache tensor直接写入到显存里。 
4. 也就是 一个seq 有一个跟它一样生命周期的kv cache tensors（甚至后者实际更长一点）。model.forward 只需要seq 以后得token id list + 对应的kvcaceh 即可work，产出是新token id list 及其对应的kvcache tensor。seq 封装了一个请求所有的上下文（用户侧配置，比如温度；资源侧配置，比如kvcache tensor）
4. 疑问：从代码看，seq的python block 只是标记了一个序号，并且真实申请显存。vllm 怎么保证应用层管理的显存，跟真实运行的显存不冲突，比如底层加载模型权重占用的空间。

1. transformer 库里执行forward 时也允许提供kvcache 参数，transformer 默认用kvcache的实现可以讲下大概原理嘛？跟vllm 对kvcache的使用有何不同呢？
2. vllm 运行推理，不仅生成了新的token，也生成新token相关的kvcache tensor。新的token 通过推理方法返回得到，新token相关的kvcache tensor保存在了哪里呢？是vllm forward 时插入的kvcache 参数对应的显存空间吗？
3. 新token对应的kvcache tensor 理论上是不是挂在vllm seq group里，vllm 执行一个seq 的时候将其对应的tokenid list和kvcache 加载并推理，执行下一个seq的时候加载另一个seq的tokenid list和kvcache。难道每次新seq 对应的kvcache tensor 都copy到了vllm forward的kvcache 参数指向的显存空间，岂不是太麻烦。或者就是底层做了透明访问？可以用一个图帮忙展示下嘛？
