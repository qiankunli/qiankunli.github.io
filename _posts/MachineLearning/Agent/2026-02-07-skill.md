...
智能体拥有智能和能力，但不一定具备有效处理实际工作的专业知识（通用知识很强，专项操作能力很弱）。这促使Claude创建了Agent Skills（Procedural Knowledge）。Skills是有组织文件的集合，将领域专业知识——工作流程、最佳实践、脚本——打包成智能体可以访问和应用的格式（以文件夹为最小分发单元）。Skill 是一项重大的 Agent 工程突破：它用一种结构化的、标准化的模式来组织经验，并用渐进式加载机制来降低上下文负担。
