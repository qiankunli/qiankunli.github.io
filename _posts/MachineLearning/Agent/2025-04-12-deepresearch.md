...
2. Deep Search（深度思考）：更侧重“找”的过程，能自适应的在 Web 上主动进行多轮检索，以不断获取与任务目标匹配的高质量信息。Deep ReSearch更侧重“写”的过程（对应任务规划Planning和报告生成模块Report），它可以利用Deep Search找到的信息，进行深入地思考，分析和创造，目标是产出深刻的理解或知识。
...
之前的DeepResearch可能需要一系列工作流：要求先plan，再搜集信息，再汇总，但是后来模型能力强大以后给Agent搜索网页工具+写文件工具就能胜任这件事了，也不需要精心设计提示词。
...
1. 挣脱“固定流程”的束缚，更灵活通用。RL Agent的行为不是被规则写死的，而是根据当前任务动态生成的。这让它在面对闻所未闻的复杂问题时，有能力探索出创造性的解决方案。我们升级底层模型时，也无需重构整个Agent体系。
    1. **Workflow拼装缺点**：比如基于OpenAI/Claude去搭建“多Agent+规划器+子任务”，靠手动Prompt和条件规则，把复杂任务拆分成小模块。每换一次底层模型，整个workflow就要大改一遍，灵活性受限。
    2. SFT（模仿学习）：人工标注完整任务轨迹，Agent模仿这些轨迹，提升Agent整体的能力。但这里面收集数据很耗费人力，难以scale到大量的数据。
...
1. What RL is for
    ...
    4. DeepRetrieval训练LLM生成符合特定搜索引擎偏好的查询（就像"黑进"搜索引擎）
    5. AutoRefine奖励"搜索-思考-精炼"的迭代过程
    6. ReSum训练智能体主动总结历史交互，避免上下文溢出
    7. （多智能体）OPERA采用分级RL，为规划、分析、改写代理提供定制化奖励信号。
2. How RL is used: 优化策略（奖励设计、训练方法）
    1. ZeroSearch完全放弃SFT，在潜在空间模拟检索，实现纯RL训练
    2. AgentGym-RL通过课程学习逐步扩展交互时长，从短任务到多步推理
    3. EvolveSearch开创自我进化循环：RL生成高质量轨迹 → 蒸馏为SFT数据 → 再RL训练
    4. 奖励函数从单一到多维的演进：Outcome、Process
...
[引入实体级奖励做Deepresearch模型强化](https://mp.weixin.qq.com/s/I27ffmsEc-Fsgl7ZOy5wqA)把“搜索智能体”想象成一个查资料、找答案的模型，让它查“1997年那部电影的导演是谁？这部电影的主演后来拿了《荒野猎人》的奥斯卡最佳男主角”，它得自己搜资料、理线索、给答案。现在这模型有个大问题：之前训练它的时候，只看“最后答案对不对”—对了就给满分，错了就给零分。但很多时候模型其实快做对了：比如它正确找到了“主演是莱昂纳多”“电影是《泰坦尼克号》”，就差最后一步说导演，结果还是被算全错，相当于白忙活。所以，一个简单的办法，训练时不光看最终答案，还看模型有没有找到关键信息（比如“莱昂纳多”“泰坦尼克号”这些核心“知识点”，也就是“实体”）。哪怕答案错了，只要找对了不少关键知识点，也给它点“ partial credit”（相当于鼓励分），让它知道“方向没跑偏，就差最后一步”。PS：构造数据集的时候，可以故意把一些实体mask（用这个实体的另一种表达方式来替换）。
...
1. 选择合适的时机对情景记忆 (episodic memory) 进行压缩，在不丢失关键语义的情况下避免上下文窗口爆炸。
2. 提供了 goal参数和 guidance参数，允许开发者指明任务目标和任务指导信息。这些信息不会被ReCENT算法压缩，保证智能体长时间运行时能够维持目标。
