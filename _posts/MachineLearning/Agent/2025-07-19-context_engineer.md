...
**上下文窗口的长度，和 Agent 能稳定使用的长度，根本不是同一个量级**。这个现象叫 Context Rot——上下文腐烂。底层原因是 Transformer 的注意力机制：复杂度是 O(n²)。窗口越长，关键信号被噪声稀释得越严重。模型不是「读不到」，是「读到了但被淹没了」。上下文工程的本质，**是系统运行时的设计艺术——如何让每个 token 都服务于策略目标**(为每个决策点动态构建上下文)，如何让模型始终聚焦于主任务，如何让信息流动高效且有序。
1. 什么是“上下文”（Context）？任何可以用来描述‘实体’（人、地点、物体）状况的信息，只要这些实体与用户和应用的交互相关，都算上下文。”这个定义非常重要。不要把“上下文”狭隘地理解为对话历史记录。你当前所在的文件夹路径、你的操作系统、你正在使用的软件、甚至你过往的偏好、外部可调用的API工具……这些都是上下文。**它是情境的总和**。
2. 什么是“上下文工程”（Context Engineering）？它是指系统性地设计、收集、存储、管理和使用上下文信息，以增强机器对人类意图的理解和任务执行能力的过程。关键词是“系统性地”。它不是零敲碎打的技巧，而是一整套方法论。它的根本目的始终没变：弥合人类的高熵、模糊意图与机器所能理解的低熵、结构化指令之间的鸿沟。
PS：原则就是：能不进上下文的，都不要进。
...
上下文精准投喂，要精准（给其所需，去其所扰） ==> 容易误删隐性关键信息，破坏上下文连贯性（删除后智能体无法追溯信息来源，导致后续决策逻辑断裂），不要轻易删除或篡改模型完整的探索历史，即使是那些未被采纳的“错误”尝试 ==> 给信息带上标记，诸如`[冗余]` `[关键]`，用 “柔性标记” 替代 “刚性删除”，平衡 “上下文精简” 与 “信息完整性”。不再仅仅是撰写一段固定的System Prompt，而是一套复杂的、动态的Prompt组装机制。


## 技术要点

token 是一种边际收益递减的有限资源，token 越多，模型对关键信息的关注度反而下降，一些研究称其为 Context Rot，这是 transformer 架构本身的限制。所以 Context Engineering 的目标是找到最小的高信号 token 集合，最大化期望结果的可能性。Agent 的 context 大致分为两类：**相对静态的部分**（System Prompt、工具定义、领域知识）和**动态增长的部分**（对话历史、工具调用记录、检索到的知识）。静态部分在会话开始时就确定了，占据固定的 token 预算；动态部分随着任务执行不断膨胀。Context Engineering 需要同时管理两者。**Context Engineering 说到底就是别让模型把算力浪费在读垃圾上。静态层面控制工具数量，动态层面压缩噪音、复用经验、按目标裁剪**。静态部分适合 cache，动态部分按需注入。PS: 算是ce的一句话。这些手段和模型能力是乘法关系，模型越强，干净的 context 带来的收益越大。

1. 上下文的收集与存储
  1. 怎么收集？ 
  2. 怎么存储？
3. 上下文的管理。如何识别上下文中冗余、过期的内容，并尽可能在不影响缓存的前提下移除这些内容；何时对上下文进行压缩并保留上下文中关键的技术细节、架构决策和遗留问题。==> **上下文必须被结构化管理**。PS： 工程上，有一个context-server 全面负责这些。
  1. 上下文的组织：分层记忆架构；上下文隔离。
  2. 上下文的抽象，或者叫“自我烘焙” (Self-Baking)。这是一个非常形象的比喻。Agent不能只是被动地存储信息，它必须能主动地“消化”和“吸收”自己的经历，把原始上下文“烘焙”成更紧凑、更结构化的知识。这才是从“记忆”到“学习”的关键。
  3. claude 以 SYSTEM_PROMPT_DYNAMIC_BOUNDARY 来分界静态和动态上下文。把 prompt 当作可编排的运行时资源来管理。
3. 上下文的使用。
  1. 上下文的选择。上下文窗口是宝贵的资源，不能什么都往里塞。必须有一套有效的筛选机制，论文称之为“注意力之前的注意力”。你需要根据语义相关性、逻辑依赖性、新近度等因素，动态地选择最关键的信息放入窗口。静态 Context 的坑：MCP 工具爆炸；3. 动态 Context 的主要 Pattern：工具 Observation 为主。
  2. 上下文的共享。
    1. 系统内共享：多个Agent如何协作？可以通过Prompt传递、结构化消息交换，或者一个共享的“黑板”（Shared Memory）来通信。
    2. 跨系统共享：你的AI助手如何与Office套件协同？需要通过适配器（Adapter）或者统一的上下文表示协议来实现。
  3. 主动的用户需求推理：最高级的用法是，系统能通过分析长期上下文，主动预测你的潜在需求。比如，发现你总是在下午搜索咖啡店，就主动在下午推荐附近的优惠。
...
Agent = reason +act， 在工程层面来说，推理本质就是LLM Call，执行本质则是Tools Call（代码可认为是Tools的一种），而连接这二者的上下文工程（Context Engineering）则是Agent框架的核心。

ACE核心理念：不依赖模型微调，而是通过上下文的自我演化，让智能体实现“自学成长”。管理信息的可访问性、结构和时机。
...
3. 好的上下文管理，是按使用频率 + 稳定性分层
  1. 常驻层： 身份、约定、禁止项 —— 短、硬、可执行
  2.  按需加载 ：Skills 描述符常驻，完整内容触发时注入
  3. 运行时注入：时间、用户偏好、渠道 ID
  4. 记忆层：MEMORY.md，按需读取，不进系统提示
  5. 系统层：Hooks、代码规则，完全不进上下文
4. **任何能用 if/else 表达的逻辑，都不该进上下文。**Hooks、定时任务、工具的入口校验，全部交给外部系统。把上下文窗口留给真正需要 LLM 推理的事。
