...
3. 系统中需要多个专业领域(如规划器、研究员、数学专家等)，同一个 system prompt 很难让 agent 在法律条文解析和财务模型计算上都做到足够深，通才在需要专家判断的场景是软肋。
4. 单体架构在扩展新功能或修改现有功能时，往往需要对整个系统进行调整，成本高且风险大。PS：**微服务里的单体架构 vs 微服务架构**
5. 需要真正的并行执行。
6. 错误需要跨任务隔离。单 agent 的统一 context 会让一处错误的中间结果被后续所有步骤看到，错误无法被隔离。
PS： 模型可以持续强大到什么都会么？
