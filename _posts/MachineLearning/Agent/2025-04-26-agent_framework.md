...
agent 
1. 从运行时长来说有长有短，从交互形式有chatbot/copilot/workspace
2. 底层llm 相对明确：token入token出/text-in-text-out
3. 中间的framework承上启下
...
Agent架构的演化史，就是因为我们在基础大模型无法完美内化“领域知识”和高效复用“长期记忆”的背景下，不断尝试“外挂”出这些能力的。本质上就是大家对大模型如何更好的注入领域知识和记忆管理这两方面的需求，不断促进了Agent架构的演化。

## 框架

1. agent 底层都是loop
2. ce，怎么组织提示、管理对话历史、设计工具定义
3. 可观测性，Agent出了问题你能不能快速定位到是哪一步推理出了偏差、哪个工具调用返回了异常数据
...
agent = reason + act，ce 负责连接两者。
1. reason = llm call，act = tool call
2. llm call有lite llm
3. tool 包括tool的范围有业内最佳实践，取决于agent使用场景
4. ce 仍然大有可为。ce的核心引擎是agent loop，每一次是一个turn ==> Agent框架设计的核心就是在Agent Loop这个While循环中设计如何管理上下文。从loop到生产级系统有一些列工程问题：循环怎么管理状态、出错了怎么恢复、多个Agent之间怎么协作、上下文窗口快满了怎么办、工具怎么标准化接入、执行过程怎么监控和调试。
...
## 理念之争：agent vs workflow（2026已过时）
