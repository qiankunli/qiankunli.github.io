...
function call 泛指的是模型通过输出一段特定格式的字符（文本）来触发一些外部的工具。
...
## llm function-call的能力判定

除了竞技场（让用户匿名投票比较两个模型结果的好坏）的评价方式，大部分针对模型能力的评测都通过预先设计好的题目进行。 同样的，在开发迭代过程中，function call 能力也可以通过一系列题目进行评测，题目的形式是给定一系列工具（function），然后提出一个问题（query），看模型能否进行正确的工具调用。 在这个基础上，题目的难度和复杂度可以从多个维度进行升级以增加任务难度和多样性，比如
1. 单问题多工具调用（不同参数 / 不同工具）
2. 多轮对话中的工具调用（与用户多轮交互 / 与环境多轮交互）
3. 缺失必要条件下的工具调用（拒绝调用，向用户询问更多信息，比如要求订机票但没给时间）
4. workflow角度增加难度（比如预先不给定合适的工具，需要模型先进行搜索操作）

大模型工具调用的能力，核心还是取决于大模型的训练数据和训练方式，但最核心的还是如何获取大规模真实的工具调用数据。ACEBench《ACEBench: Who Wins the Match Point in Tool Usage?》涵盖8大领域和68个子领域，包括技术、金融、娱乐、社会、健康、文化、环境等，共包含4,538个API，中英文均可。
...
### 利用functioncall来做数据提取

比如想提取一段话里的城市，则可以给一个查询天气的tool，参数是city，则llm在给tool 构造参数的过程中，也就自然提取了天气。
