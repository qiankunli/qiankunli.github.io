...
Agent 的四大能力：PS：每谈论一个东西，这个东西就有它固有的脉络，按脉络走就都梳理清楚了。
...
4. 执行。Action 能力强烈依赖于基座模型的 function calling 能力，即理解任务并依据 API 描述将自然语言的任务转换为 API request 的能力。PS：选择哪个模型是关键因素
    1. 由于执行准确率有限，要考虑到 function calling 失败的备选方案，比如：
    1. - 客服案例中，function calling 失败直接对接到人工。
    2. - API 参数缺失，考虑根据 API 返回的错误信息，转换成用户语言继续追问用户。
2. 反省，反思能力依赖于它的记忆能力。
一方面感觉 LLM 无所不能、充满希望，另一方面又感觉每个地方都差那么一点点。每一步的准确率漏斗