...
OOD/Out-of-Domain
1. 预检索阶段
    1. 给提问贴意图标签。将用户查询映射到预定义的业务意图空间，不在空间内的直接拒答。
    1. 动态查询重写
2. 检索阶段
    1. 过滤低置信度匹配。传统做法设定固定余弦相似度阈值（如0.75），但存在致命缺陷：不同Embedding模型的向量分布差异极大，高维空间的聚类效应也会让无关文档获得虚假高分。自适应阈值的核心逻辑：分析Top-K结果的分数分布，而非绝对数值：
        1. 如果Top-1分数显著高于Top-2（陡峭落差）→ 大概率是精准匹配，保留结果；
        2. 如果Top-10分数平缓（彼此差异小）→ 系统在硬凑相关文档，判定为OOD，剔除结果。
    2. 用双重验证避免单一检索的盲区
3. 后检索阶段—— 引入相关性裁判
