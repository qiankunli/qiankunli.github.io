...
2. 数据并行：因为训练费时的一个重要原因是训练数据量很大。数据并行就是在很多设备上放置相同的模型，并且各个设备采用不同的训练样本对模型训练。训练深度学习模型常采用的是batch SGD方法，采用数据并行，可以每个设备都训练不同的batch，在反向传播时，**模型参数有多个worker副本**，为了能将结果共享——**确保整个模型参数能够在不同的GPU之间进行同步，所有的梯度都将进行全局归纳**（如梯度平均或参数服务器）。所有worker共享ps 上的模型参数，并按照相同拓扑结构的数据流图进行计算。
