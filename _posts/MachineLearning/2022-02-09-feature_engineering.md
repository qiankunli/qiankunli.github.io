...
4. 框架支持将Hive表里的记录映射成Domain对象，并将序列化后的结果写入KV存储
    ![](/public/upload/machine/feature_offline_push_online.png)
2. 日志落盘的是物料数据。这样离线环节，只需要将物料数据和曝光点击等行为日志关联标签，继而再使用特征算子包计算生产特征样本，即可进行模型训练。线上和线上基于同样的特征算子包做计算以保证预估和训练的一致性。用配置化的方式描述模型特征计算和转换过程，通过特征配置化管理能力、特征&算子&解决方案复用能力以及离线在线打通能力，提升了特征迭代效率

    ![](/public/upload/machine/feature_offline_online_configure.png)
